  ...
  return (
    ...
    >
      <div className="relative h-[80%] aspect-square rounded-[1vh] overflow-hidden bg-bg-card flex-shrink-0 mr-[3vw] flex items-center justify-center border border-border">
        {song.coverUrl ? (
          <img loading="lazy" src={song.coverUrl} alt="" className="w-full h-full object-cover" />
        ) : (
...
export function MusicList({ isPortrait = true }: { isPortrait?: boolean }) {
  ...
  return (
    <div className="flex-1 overflow-y-auto no-scrollbar relative w-full h-full pb-[10vh]" style={{ contain: 'content' }}>
        ...
        <div className="px-[2vw]">
          <AnimatePresence initial={false}>
            {queue.map((song, index) => {
              if (hiddenSongs.includes(song.id)) return null;
              return (
                <SongItem
                  key={song.id}
                  song={song}
